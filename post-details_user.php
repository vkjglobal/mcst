...
if(isset($_GET['id'])){
  ...
  //echo "<pre/>";print_r($res);
    ...
    $public_visisble     =   $res['file_public_visible'] ;
     $mem_visisble       =   $res['mem_hide_status'] ;
   ...
   if((empty($postImage)) || (!file_exists($postImageURL))) {                                        
    ...
    if($public_visisble == 0){
     ...
     if($mem_visisble == 0){
                                            $publish_mem    =   "Visible To Other Members"; //hidden from other members
                                            $status_change_need_mem =   1;// update to 1 on db needed
                                        } else{
                                             $publish_mem    = "Hidden from Other Members";
                                            $status_change_need_mem =   0;// update to 0 on db needed
                                        }
//=========comments====================
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            <div class="d-flex w-100 justify-content-end">
                                ...
                                <button type="button" class="btn border btn-typ4 px-4 ms-2"  onclick="publicMEmStatus(<?php echo $id; ?>,<?php echo $status_change_need_mem;?>)"><?php echo $publish_mem; ?></button>
                                <!--  <button type="button" class="btn border btn-typ4 px-4 ms-2" onclick="publicStatus(<?php echo $id; ?>, <?php echo $status_change_need ? 'true' : 'false'; ?>)"><?php echo $publish; ?></button>  -->
                                 ...
                                 <a href="post-details_user_edit.php?id=<?php echo $_GET['id']; ?>" class="btn border btn-typ4 px-4 ms-2">
                                   Edit
                                </a>
...
function publicStatus(postId,publicStatuschange){
                  ...
                  $.ajax({
                    ...
                    //==============visible status to members
                    function publicMEmStatus(postId,publicStatuschangeMem){
    
                  $.ajax({
                            url: 'ajax_mypost.php', // Replace with your form processing script
                            type: 'POST',
                         //data: { firstEdit: firstEdit, secondEdit: secondEdit,bannerImageEdit: filename ,homeId: homeId,oldImage: oldImage },
                         data: { MemPostId: postId,publicStatuschangeMem: publicStatuschangeMem},
                        success: function(response){  
