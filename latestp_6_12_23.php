...
try {
    $query1 = "SELECT * FROM `cms` WHERE `menu__-` LIKE 'Latest Presentat%'";
...
<body>
    <section class="page-title-section mb-lg-5 mb-4">
        <div class="container">
            <h2 class="hd-typ1">Latest Presentations</h2>
        </div>
    </section>
    <section class="news-section mb-5 pb-xl-5 pb-lg-4 pb-md-3">
        <div class="container">
            ...
            <ol class="post-list-bottom">
                <?php
                // Check if there are more items in the result set to display in the link section
                if (count($history) == $itemsPerPage && $currentPage < $totalPages) {
                    $nextPage = $currentPage + 1;
                    $nextPageOffset = $offset + $itemsPerPage;
                    $nextPageQuery = "SELECT * FROM `cms` WHERE `menu__-` LIKE 'Latest Presentat%' LIMIT $nextPageOffset, $itemsPerPage";
